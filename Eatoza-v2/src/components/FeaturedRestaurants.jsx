...
const FeaturedRestaurants = ({ filters, onSelectRestaurant, onClearFilters, currentUserId }) => {
  ...
  useEffect(() => {
    ...
    // ✅ FIXED (removed http://)
    const url = `${import.meta.env.VITE_API_URL}/api/restaurants?${params.toString()}`;
...
//     console.log('Fetching:', `${import.meta.env.VITE_API_URL}/api/restaurants?${params.toString()}`);

//     fetch(`${import.meta.env.VITE_API_URL}/api/restaurants?${params.toString()}`)
//       .then(res => res.json())
//       .then(data => {
//         if (data.success) setRestaurants(data.data);
//         else setError('Failed to load restaurants');
