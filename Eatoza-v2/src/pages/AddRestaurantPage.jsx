...
const AddRestaurantPage = ({ navigate }) => {
  ...
  const toggleDay = (d) => setForm(f => ({
    ...f, daysOpen: f.daysOpen.includes(d) ? f.daysOpen.filter(x => x !== d) : [...f.daysOpen, d]
  }));
  const nextStep = () => setStep(s => Math.min(s + 1, 3));
  const prevStep = () => setStep(s => Math.max(s - 1, 0));

  const handleSubmitClick = () => {
    if (!isSignedIn) { setError('You must be logged in to add a restaurant.'); return; }
    if (!form.name || !form.city || !form.cuisines.length) {
      setError('Please fill in restaurant name, city and at least one cuisine.'); return;
    }
    setError('');
    setShowOtp(true);
  };
    ...
    return (
      <div className="ar-page">
        ...
        <div className="ar-success">
          <div className="success-icon">🎉</div>
          <h2>Restaurant Added!</h2>
          <p><strong>{form.name}</strong> has been successfully added to Eatoza and is now visible to customers!</p>
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
            <button className="ar-primary-btn" onClick={() => navigate('home')}>Back to Home</button>
            <button className="ar-secondary-btn" onClick={() => {
              setSubmitted(false); setStep(0);
              setForm({ name: '', tagline: '', cuisines: [], phone: '', email: '',
                city: '', address: '', pincode: '', openTime: '09:00', closeTime: '22:00',
                daysOpen: [], avgPrice: '', deliveryTime: '', minOrder: '', description: '', image: '' });
            }}>Add Another</button>
          </div>
        </div>
      </div>
        ...
        <div className="ar-form-card">
          ...
          {step === 0 && (
            <div className="ar-step">
              ...
              <div className="ar-form-grid">
                <div className="ar-field full">
                  <label>Restaurant Name *</label>
                  <input name="name" value={form.name} onChange={handle} placeholder="e.g. Spice Garden" />
                </div>
                <div className="ar-field full">
                  <label>Tagline</label>
                  <input name="tagline" value={form.tagline} onChange={handle} placeholder="e.g. Authentic flavours from the heart of India" />
                </div>
                <div className="ar-field full">
                  <label>Image URL</label>
                  <input name="image" value={form.image} onChange={handle} placeholder="https://images.unsplash.com/..." />
                  <small style={{ color: '#888', marginTop: '4px' }}>Paste any image URL from Unsplash</small>
                </div>
                <div className="ar-field">
                  <label>Phone Number</label>
                  <input name="phone" value={form.phone} onChange={handle} placeholder="+91 99999 99999" />
                </div>
                <div className="ar-field">
                  <label>Email</label>
                  <input name="email" type="email" value={form.email} onChange={handle} placeholder="restaurant@example.com" />
                </div>
              </div>
              <div className="ar-field full">
                <label>Cuisines * (select all that apply)</label>
          ...
          {step === 2 && (
            <div className="ar-step">
              ...
              <div className="ar-form-grid">
                <div className="ar-field">
                  <label>Average Cost for Two (₹)</label>
                  <input name="avgPrice" value={form.avgPrice} onChange={handle} placeholder="e.g. 500" type="number" />
                </div>
                <div className="ar-field">
                  <label>Avg. Delivery Time (mins)</label>
                  <input name="deliveryTime" value={form.deliveryTime} onChange={handle} placeholder="e.g. 30" type="number" />
                </div>
                <div className="ar-field">
                  <label>Minimum Order (₹)</label>
                  <input name="minOrder" value={form.minOrder} onChange={handle} placeholder="e.g. 150" type="number" />
                </div>
              </div>
              <div className="ar-field full">
                <label>About Your Restaurant</label>
          ...
          {step === 3 && (
            <div className="ar-step">
              ...
              {error && <p className="ar-error">{error}</p>}
              <p className="review-note">✅ By submitting, you agree to Eatoza's Partner Terms and Conditions.</p>
            </div>
          )}
          ...
          <div className="ar-actions">
            ...
            {step < 3
              ? <button className="ar-primary-btn" onClick={nextStep}>Next Step →</button>
              : <button className="ar-primary-btn submit" onClick={handleSubmitClick} disabled={loading}>
                  {loading ? 'Saving...' : '🚀 Submit Restaurant'}
                </button>
            }
          </div>
